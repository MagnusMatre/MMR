...
namespace test {
		...
		m_curModelName(""), m_mvp(glm::mat4(1.0f)), m_view(glm::mat4(1.0f)), m_wireFrameColor(glm::vec3(0.0f, 0.0f, 0.0f)), m_wireFrameWidth(1.0f)
	{
		...
		m_okayDirectory = "../../data/OkayMeshes";
		m_normalflipDirectory = "../../data/NormalflipMeshes";
		m_smallchangeDirectory = "../../data/SmallchangeMeshes";
		m_hopelessDirectory = "../../data/HopelessMeshes";

		//m_refinedMeshPathOutput = "C:/Users/timgr/Documents/GitHub/MMR/data/RefinedMeshes/AircraftBuoyant/name";
		m_dataRoot = "../../data";
		m_curDirectory = m_dataRoot + "/VCGdecimatedCleaned";
	...
	void TestClassMeshes::loadNewMesh(const std::string& path) {
		...
		try {
			...
			std::cout << m_hopelessDirectory << " " << fileName << " " << directoryName << std::endl;

			std::string outputFile = m_hopelessDirectory + "/" + directoryName + "/" + fileName;
    ...
    void TestClassMeshes::OnImGuiRender() {
		...
		// Button for saving the mesh to manually save mesh to the okay folder
		if (m_curModelName != "" && ImGui::Button("IS OKAY")) {
			std::filesystem::path filePath(m_curModelName);
			std::string fileName = filePath.filename().string();
			std::string directoryName = filePath.parent_path().filename().string();

			std::cout << m_okayDirectory << " " << fileName << " " << directoryName << std::endl;

			std::string outputFile = m_okayDirectory + "/" + directoryName + "/" + fileName;
			m_curGeomMesh->SaveMesh(outputFile);
			ProceedToNextMesh();
		}

		if (m_curModelName != "" && ImGui::Button("NEEDS NORMAL FLIP")) {
			std::filesystem::path filePath(m_curModelName);
			std::string fileName = filePath.filename().string();
			std::string directoryName = filePath.parent_path().filename().string();

			std::cout << m_normalflipDirectory << " " << fileName << " " << directoryName << std::endl;

			std::string outputFile = m_normalflipDirectory + "/" + directoryName + "/" + fileName;
			m_curGeomMesh->SaveMesh(outputFile);
			ProceedToNextMesh();
		}

		if (m_curModelName != "" && ImGui::Button("NEEDS SMALL CHANGES")) {
			std::filesystem::path filePath(m_curModelName);
			std::string fileName = filePath.filename().string();
			std::string directoryName = filePath.parent_path().filename().string();

			std::cout << m_smallchangeDirectory << " " << fileName << " " << directoryName << std::endl;

			std::string outputFile = m_smallchangeDirectory + "/" + directoryName + "/" + fileName;
			m_curGeomMesh->SaveMesh(outputFile);
			ProceedToNextMesh();
		}

		// Button for saving the mesh to manually save mesh to the hopeless folder
		if (m_curModelName != "" && ImGui::Button("HOPELESS")) {
			std::filesystem::path filePath(m_curModelName);
			std::string fileName = filePath.filename().string();
			std::string directoryName = filePath.parent_path().filename().string();
