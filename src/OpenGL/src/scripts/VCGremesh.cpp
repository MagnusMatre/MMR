...
void VCGremesh::edge_collapse(MyMesh& mesh) {
	...
	while (cur_num_vertices > m_target + m_threshold) {
...
void VCGremesh::remesh_all_directories(std::string& start_dir){
	for (const auto& class_folder : std::filesystem::directory_iterator(m_input_dir)) {
		...
		//Open a log file names "remeshLog.txt" to write the output log to2
		...
		for (const auto& entry : std::filesystem::directory_iterator(m_input_dir + "/" + directoryName)) {
			...
			std::cout << "Saving mesh to file: " << m_output_dir << "/" << entry.path().filename().string() << " with " << mesh.VN() << " vertices" << std::endl;
			logFile << "Saving mesh to file: " << m_output_dir << "/" << entry.path().filename().string() << std::endl;

			if (vcg::tri::io::Exporter<MyMesh>::Save(mesh, outputFile.c_str()) != 0) {
...
void VCGremesh::remesh_directory() {
	...
	for (const auto& entry : std::filesystem::directory_iterator(directoryPath)) {
		...
		if (vcg::tri::io::Exporter<MyMesh>::Save(mesh, outputFileName.c_str()) != 0) {
