...
void QueryBenchmark::RunBenchmark(int K, STANDARDIZATION_TYPE s_type, STANDARDIZATION_TYPE s_type_histogram, DISTANCE_TYPE d_type_scalar, DISTANCE_TYPE d_type_histogram, float gamma) {
	...
	for (int i = 0; i < 16; i++) {
		...
		cur_total_corrected_score += m_frequencies[i] * (cur_score / 16);
	...
	compact_results_file << std::to_string(s_type) << std::to_string(s_type_histogram) << std::to_string(d_type_scalar) << std::to_string(d_type_histogram) << "_" << std::to_string(gamma) << " " << cur_total_score << " " << cur_total_corrected_score << std::endl;
	...
	for (int it = 0; it < max_iterations; it++) {
		...
		while (new_score <= cur_score) {
			...
			for (int k = 0; k < NUM_HISTOGRAM_FEATURES; k++) {
				...
				int u = std::rand() % 4;
				if (u == 0) {
					m_queryEngine->TuneHistogramWeights((HISTOGRAM_FEATURES)k, m_queryEngine->m_histogram_weights[k] * cur_factor);
				}
				else if (u == 1) {
					m_queryEngine->TuneHistogramWeights((HISTOGRAM_FEATURES)k, m_queryEngine->m_histogram_weights[k] / cur_factor);
				}
			...
			if (u == 0) {
				m_queryEngine->TuneGamma(gamma * 1.05 );
			}
			else if (u == 1) {
				m_queryEngine->TuneGamma(gamma / 1.05);
			}
			...
			// After the count reaches 10, increase the factor by which we scale the weights
			if (cnt % 10 == 0) {
				cur_factor *= 1.1;
			}
...
double QueryBenchmark::computeScore() {
	...
	for (int i = 0; i < 16; i++) {
		...
		cur_total_score += (double)cur_score / 16;
	}
